...
# podman run --userns=keep-id -d --name psg -p 127.0.0.1:8080:8080 --volume ~/my-var-run:/var/run --volume ./tempConf.conf:/etc/httpd/conf/httpd.conf --volume ./tempwww.conf:/etc/php-fpm.d/www.conf psgimage
# podman run --userns=keep-id -d --name psg -p 127.0.0.1:8080:8080 --volume ~/my-var-run:/var/run:rw --volume ./tempConf.conf:/etc/httpd/conf/httpd.conf --volume ./tempwww.conf:/etc/php-fpm.d/www.conf psgimage

# apptainer instance start \ 
#     -B ~/my-var-run:/var/run:rw \ 
#     -B ./tempConf.conf:/etc/httpd/conf/httpd.conf \
#     -B ./tempwww.conf:/etc/php-fpm.d/www.conf \ 
...
# apptainer instance start \
#   -B ~/my-var-run:/var/run:rw \
#   -B ./tempConf.conf:/etc/httpd/conf/httpd.conf:rw \
#   -B ./tempwww.conf:/etc/php-fpm.d/www.conf:rw \
#   -B ~/apache_logs:/etc/httpd/logs:rw \
#   -B ~/phpfpm_logs:/var/log/php-fpm:rw \
#   -B ./tempResults:/var/www/html/results:rw \
#   workingPSG.sif psg

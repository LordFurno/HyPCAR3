...
class customLoss(nn.Module):
    ...
    def forward(self,predAbun,realAbun,detectionProb):
        mseLoss=self.dataBased(predAbun,realAbun)

        #if detection says “present”
        #punish pred_abun<0.005
        #hinge_present = max(0, 0.005 − predAbun)
        hinge_present=F.relu(0.001 - predAbun)

        present_penalty=(detectionProb*hinge_present).mean()

        #if detection says “absent”
        #punish pred_abun > 0.2
        #hinge_absent = max(0, pred_abun − 0.2)
        hinge_absent = F.relu(predAbun - 0.2)
        absent_penalty = ((1.0 - detectionProb) * hinge_absent).mean()

        # 4) Combine
        constraint_loss = present_penalty + absent_penalty
        total_loss = mseLoss + constraint_loss
...
if __name__ == '__main__':
    ...
    optimizer=optim.Adam(model.parameters(),lr=0.0011258209507438782)
    ...
    for epoch in range(numEpochs):
        ...
        for batch in trainingDataloader:
            ...
            loss=criterion(outputs,labels,detectionOutput)
            running_loss+=loss
        ...
        with open("finalBaseAbundance.txt",'a') as f:
        ...
        with torch.no_grad():
            ...
            for index,batch in enumerate(validationDataloader):
                ...
                loss=criterion(outputs,labels,moleculeDetection)
    ...
    torch.save(model.state_dict(), "finalBaseAbundance.pt")
    model.eval()

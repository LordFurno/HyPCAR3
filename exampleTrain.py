...
def soft_prior_mean(predAbun, tau=1e-2):
    ...
    #Calculate all four outputs, zeroing out those invalid by side-conditions
    D1 = H + 2*O + 2*N + 1e-6
    D2 = 2*H + 4*O + 4*N + 1e-6
    muH2O = (H + 2*O - 4*C) / D1
    muCH4 = (H - 2*O + 4*C) / D2
    ...
    #7) final mixture
    gates = torch.stack([gateA1, gateA2, gateB, gateC, gateU], dim=1) 
    mus   = torch.stack([mu_A1, mu_A2, mu_B, mu_C, mu_U], dim=2)     

    mu = torch.sum(mus * gates.unsqueeze(1), dim=2) 
    return mu
...
def calculatePrior(predAbun, sigmaPrior):
    ...
    mu = soft_prior_mean(predAbun)  

    #2) truncated normal cdf gap
    normal = torch.distributions.Normal(0., 1.)
    a = (0.0 - mu)           / sigmaPrior
    b = (1.0 - mu)           / sigmaPrior
    Z = normal.cdf(b) - normal.cdf(a)       
...
def project_onto_simplex(v):
    ...
    v_cumsum    = v_sorted.cumsum(dim=1)                   

    #2) find rho
    js = torch.arange(1, N+1, device=v.device).view(1, -1)  
    rho_candidates = v_sorted + (1 - v_cumsum) / js         
    ...
    rho = mask.to(torch.int).argmax(dim=1) + 1              

    #3) compute theta
    idx   = rho - 1                                         
    theta = (1 - v_cumsum[torch.arange(B), idx]) / rho      

    #4) project
    return torch.clamp(v + theta.view(B,1), min=0.0)        
...
class PSGSPSAFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_T):
        ...
        prod = grad_T * dT_dir                 
        D = prod.sum(dim=[1,2], keepdim=True)  
        D = D.view(batch, 1)                   
...
if __name__ == '__main__':
    ...
    for atmosphereType in ["A","B","C"]:
        dataPath=f"\\{atmosphereType}"

        curFolderPath=r"C:\Users\Tristan\Downloads\HyPCAR3\data"+dataPath
        configFolderPath=r"C:\Users\Tristan\Downloads\HyPCAR3\configFiles"
    ...
    model.load_state_dict(torch.load("finalBaseAbundance.pt",weights_only=True))
    ...
    for epoch in range(numEpochs):
        ...
        physics_batches = set(random.sample(range(num_batches), k=25))
        model.train()
        start=time.time()
        running_loss = 0.0
        running_KL_loss = 0.0  
        ...
        for batchIndex,batch in enumerate(trainingDataloader):
            ...
            if batchIndex in physics_batches:
                ...
                loss=criterion(outputs,uncertainties,labels,detectionOutput,configs,data)
            else:
                ...
                loss = ( dataLoss  * criterion.w_data+
                    + detLoss   * criterion.w_det
                    + priorNLL  * criterion.w_prior)

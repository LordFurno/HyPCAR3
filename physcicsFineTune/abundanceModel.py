...
def detectMolecules(data):
    ...
    detect.load_state_dict(torch.load("~/projects/def-pjmann/tristanb/flexibleDetectionModel.pt",weights_only=True))
    with torch.no_grad():
        outputs=detect(data)
...
class customLoss(nn.Module):
    ...
    def forward(self,predAbun,uncertainty,realAbun,detectionOutput,config,inputTransmittance):#Predicted abundances, actual abundances, config file, real data
        ...
        posteriorLambda=(1/posterior)#No divide by 3, since I"m saying this is 3x more important
        dataLossLambda=(1/dataLoss)/3
        detectionLambda=(1/detectionLoss)/3

        #Combine the loss
        #For now, just add everything, but the in future, can play with the idea of multiplying each by some factor 
        return (posterior*posteriorLambda)+(dataLoss*dataLossLambda)+(detectionLoss*detectionLambda)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("~/projects/def-pjmann/tristanb/finalBaseAbundance.pt",weights_only=True))
